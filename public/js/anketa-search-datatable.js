$(function () {
    let table = $('#datatable').DataTable({
         ...
         columns: [
            {data: 'actions', searchable: false, orderable: false, render: function(data, type, row) {
                ...
                if($('#datatable').data('route-destroy-user-can') == 'can') {
                    ...
                    <a class="btn btn-icon btn-light btn-hover-primary btn-sm mx-2 modal-delete" data-uuid=${row.uuid}>
                ...
                return `
                    <a href="${$('#datatable').data('route-show') + '/'+ row.uuid}" 
                    ...
                    <a href="${$('#datatable').data('route-edit').replace("//edit", "/"+row.uuid+"/edit")}" 
                    class="btn btn-icon btn-light btn-hover-primary btn-sm mx-2">
                        <span class="svg-icon svg-icon-md svg-icon-primary">
        ...
        "drawCallback": function( settings ) {
            ...
            show_modal_delete('Вы действительно хотите удалить анкету ','data-id');
            console_alerts();
        }
    });

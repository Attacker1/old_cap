...
class Init
{
    ...
    {
        if (!empty(config('layout-m.page-loader.type'))) {
    ...
    {
        if (config('layout-m.header.self.fixed.desktop')) {
        ...
        if (config('layout-m.header.self.fixed.mobile')) {
            Metronic::addClass('body', 'header-mobile-fixed');
            Metronic::addClass('header-mobile', 'header-mobile-fixed');
        }

        // Menu
        if (config('layout-m.header.menu.self.display')) {
            Metronic::addClass('header_menu', 'header-menu-layout-' . config('layout-m.header.menu.self.layout'));

            if (config('layout-m.header.menu.self.root-arrow')) {
                Metronic::addClass('header_menu', 'header-menu-root-arrow');
            }
        }

        if (config('layout-m.header.self.width') === 'fluid') {
    ...
    private static function initSubheader()
    {
        ...
        $subheader_style = config('layout-m.subheader.style');
        $subheader_fixed = config('layout-m.subheader.fixed');

        // Fixed content head
        if (config('layout-m.subheader.fixed') && config('layout-m.header.self.fixed.desktop')) {
    ...
    private static function initContent()
    {
        ...
        if (config('layout-m.content.width') == 'fluid') {
    ...
    {
        if (config('layout-m.aside.self.display') != true) {
            return;
        }
        ...
        // Fixed Aside
        if (config('layout-m.aside.self.fixed')) {
        ...
        // Default fixed
        if (config('layout-m.aside.self.minimize.default')) {
            Metronic::addClass('body', 'aside-minimize');
        }

        // Menu
        // Dropdown Submenu
        if (config('layout-m.aside.menu.dropdown') == true) {
        ...
        if (config('layout-m.aside.menu.submenu.dropdown.hover-timeout')) {
            Metronic::addAttr('aside_menu', 'data-menu-dropdown-timeout', config('layout-m.aside.menu.submenu.dropdown.hover-timeout'));
        }
    }
    ...
    {
        // Fixed header
        if (config('layout-m.footer.fixed') == true) {
            Metronic::addClass('body', 'footer-fixed');
        }

        if (config('layout-m.footer.width') == 'fluid') {

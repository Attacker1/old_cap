...
@section('content')
    ...
    <form name="form" method="post" action="{{ route('admin.catalog.categories.create') }}">
        {{ csrf_field() }}
    ...
    <div class="row">
        ...
        <div class="input-group mb-3 col-sm-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('comment') }}"  class="form-control " autocomplete="off" required aria-label="" aria-describedby="name_label">
        </div>
        ...
        <div class="input-group mb-3 col-sm-4">
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}"  class="form-control " autocomplete="off" required aria-label="" aria-describedby="slug_label">
        </div>

        <div class="input-group mb-3 col-sm-4">
            <div class="input-group-prepend">
                <span class="input-group-text" id="car">Род. категория:</span>
            </div>
            ...
            @if($attributes)
                @foreach($attributes as $k=>$v)
                    <div class="form-group custom-control custom-switch custom-control-inline">
                        <input type="checkbox" name="attribute_id[{{ @$v }}]" value="{{ @$k }}" class="custom-control-input" id="attributeID{{ @$k }}" >

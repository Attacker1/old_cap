...
@section('content')
    ...
    <div class="row justify-content-center mb-4">
        <form method="POST" action="{{route('clients-statuses.store')}}" class="col-md-7">
        {{ csrf_field() }}
            <div class="form-group">
                <label for="inputUserName">Название</label>
                <input type="text" name="name" class="form-control" value="{{ old('name') }}" id="inputClientName" aria-describedby="nameHelp" placeholder="Название" required autofocus>
                @if ($errors->has('name'))

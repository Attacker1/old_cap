...
@section('content')
    ...
    <form name="form" method="post" action="{{ route('client.settings.update',$data->id) }}">
        {{ csrf_field() }}
                                ...
                                <div class="col-sm-12 parent">
                                    <div class="row ">
                                        ...
                                        <div class="input-group mb-1 col-sm-4">
                                            <div class="input-group-prepend"><span class="input-group-text" id="preset_label">{{ @$k }}</span></div>
                                            <input class="form-control" name="params[{{$k}}]" value="{{ @$v }}" placeholder="Значение" autocomplete="off" required>
                                        </div>

                                        <div class="col-sm-1 mb-1  form-group">
                                            <button type="button" class="btn btn-sm btn-outline-danger delete_row__"><i class="fa far fa-trash-alt text-danger"></i></button>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '#add_condition', function () {
                ...
                html =  '<div class="col-sm-12 parent"><div class="row ">' +
                    ...
                    '<div class="col-sm-1 mb-1 form-group"><button type="button" class="btn btn-sm btn-outline-danger delete_row__"><i class="fa far fa-trash-alt text-danger"></i></button></div>' +
                    ' </div></div>';

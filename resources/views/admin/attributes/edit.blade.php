...
@section('content')
    ...
    <form name="form" method="post" action="{{ route('admin.catalog.attributes.edit',$data->id) }}">
        {{ csrf_field() }}
        ...
        <div class="input-group mb-1 col-sm-3"> <div class="input-group-prepend"><span class="input-group-text" id="preset_label">сет</span></div>
            <select name="preset_id" class="form-control" aria-label=""
                ...
                @foreach($presets as $k=>$v)
                    <option value="{{ $k }}" @if($data->preset_id == $k) selected @endif> {{ $v }}</option>
                @endforeach </select>
        </div>
                                ...
                                <div class="col-sm-12 parent">
                                    <div class="row ">
                                        ...
                                        <div class="input-group mb-1 col-sm-4">
                                            ...
                                            <input class="form-control" name="params[value][]" value="{{ @$params['value'][$k] }}" placeholder="Значение" autocomplete="off" required>
                                        </div>

                                        <div class="col-sm-1 form-group">
                                            <button type="button" class="btn btn-sm btn-outline-danger delete_row__"><i
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '#add_condition', function () {
                ...
                html =  '<div class="col-sm-12 parent"><div class="row "><div class="input-group mb-1 col-sm-4"> ' +
                    '<div class="input-group-prepend"><span class="input-group-text" id="preset_label">значение</span></div>' +
                    '<input class="form-control" name="params[value][]" value="" placeholder="Значение" autocomplete="off" required></div>' +
